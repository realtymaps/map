...
CREATE INDEX  parcel_prior_entries_idx ON parcel USING gin (prior_entries);
...
-- migrate current legacy mv_parcel data to new table
...
INSERT INTO parcel (
  ...
  select distinct on (rm_property_id)
    ...
    geom_point_json,
    fips_code::int,
    'mv_parcels' data_source_id,
    '-1' batch_id,
    't' active
  from mv_parcels
  where
  rm_property_id is not null and
  street_address_num is not null and
  geom_polys_raw is not null and
  geom_polys_json is not null and
  geom_point_raw is not null and
  geom_point_json is not null and
  fips_code is not null
);

DROP TABLE IF EXISTS mv_parcels;
