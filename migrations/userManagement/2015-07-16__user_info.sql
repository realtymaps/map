...
insert into account_use_types (type, description) values ('realtor', 'I\'m a realtor.');
insert into account_use_types (type, description) values ('real estate developer', 'I\'m real estate developer.');
insert into account_use_types (type, description) values ('real estate investor', 'I\'m real estate investor.');
insert into account_use_types (type, description) values ('property manager', 'I\'m property manager.');
insert into account_use_types (type, description) values ('own residence', 'I\'m doing research to buy or sell my own residence.');
...
ALTER TABLE auth_user_profile ADD CONSTRAINT "fk_auth_user_account_image_id" FOREIGN KEY (account_image_id) references account_images(id) ON UPDATE CASCADE ON DELETE SET NULL,
...
ALTER TABLE auth_user ADD COLUMN us_state_id int4;
ALTER TABLE auth_user ADD CONSTRAINT "fk_user_us_state_id" FOREIGN KEY (us_state_id) REFERENCES auth_user (id) ON UPDATE CASCADE ON DELETE SET NULL;
