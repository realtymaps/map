CREATE OR REPLACE FUNCTION jq_get_queue_needs()
  ...
  BEGIN
    ...
    SELECT
      ...
      CASE
        -- if all we have left are zombies, let heroku kill them; but otherwise we need to count them to
        -- prevent deadlock, and likewise need to round up
        WHEN COUNT(*) = COUNT(subtasks.status = 'zombie' OR NULL) THEN 0
        WHEN NOT jq_queue_config.active THEN 0
        ELSE
