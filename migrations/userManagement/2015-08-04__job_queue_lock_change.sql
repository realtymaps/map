CREATE OR REPLACE FUNCTION jq_get_next_subtask(source_queue_name TEXT) RETURNS jq_current_subtasks AS
  ...
  BEGIN
    ...
    PERFORM pg_advisory_lock(jq_lock_key(), queue_lock_id);        -- get a lock on the queue...
    ...
    PERFORM pg_advisory_unlock(jq_lock_key(), queue_lock_id);           -- release the lock on the queue.
