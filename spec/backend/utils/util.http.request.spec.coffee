...
describe 'utils/http.request.validateAndTransform()'.ourNs().ourNs('Backend'), ->
  ...
  promiseIt 'should reject if any parameters fail subvalidation', () ->
    expectReject(validateAndTransform({a: 'abc', b: '5.2'}, {a: validators.string(forceUpperCase: true), b: validators.float(max: 5.1)}), DataValidationError)
  ...
  promiseIt 'should resolve with defaults for required parameters, or reject if a default value is not given', () ->
    [
      ...
      expectReject(validateAndTransform({a: '', b: '0'}, {}, {c: undefined}), DataValidationError)
    ]
  ...
  promiseIt 'should perform iterative validation when a validator is an array', () ->
    [
      ...
      expectReject(validateAndTransform({a: null}, {a: [validators.defaults(defaultValue: '123'), validators.integer(max: 100)]}), DataValidationError)
      # even nested arrays
