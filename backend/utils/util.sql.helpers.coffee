...
expectSingleRow = (rows, quiet) ->
  if !rows?.length
    throw new ExpectedSingleRowError({quiet}, 'Expected a single result and rows are empty!')
  if !rows[0]?
    throw new ExpectedSingleRowError({quiet}, "Expected a single result and row is #{rows[0]}")  # undefined or null
  return rows[0]
