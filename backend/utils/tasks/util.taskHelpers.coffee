...
loadRetsTableUpdates = (subtask, options) ->
  ...
  .catch isUnhandled, (error) ->
    throw PartiallyHandledError(error, "login to RETS server failed")
  .then () ->
  ...
  .catch isUnhandled, (error) ->
    throw PartiallyHandledError(error, "failed to determine table fields")
  .then (fields) ->
    ...
    .then () ->
      ...
      dbs.properties.knex.schema.createTable rawTableName, (table) ->
        ...
        _.forEach fields, (field) ->
          table.text(field)
    .catch isUnhandled, (error) ->
      throw PartiallyHandledError(error, "failed to create temp table: #{rawTableName}")
    .then () ->
    ...
    .catch isUnhandled, (error) ->
      ...
      throw PartiallyHandledError(error, "failed to query RETS system")
    .then (results) ->
      ...
      .finally () ->
        ...
        pgClient.end()
    .catch isUnhandled, (error) ->
      throw PartiallyHandledError(error, "failed to stream raw data to temp table: #{rawTableName}")
  .finally () ->
