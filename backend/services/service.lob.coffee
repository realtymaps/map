...
getPriceQuote = (userId, campaignId) ->
  ...
  tables.mail.campaign()
    .select('id', 'auth_user_id', 'name', 'preview_content', 'aws_key', 'status', 'sender_info', 'recipients', 'options', 'custom_content')
  ...
  .then ([campaign]) ->
    ...
    if !campaign.aws_key
      ...
      address_property = getAddress(campaign.recipients[0].property || campaign.recipients[0])
      macros = getMacroData(campaign, address_to, address_from, address_property)
