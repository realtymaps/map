...
for control in directiveControls
  do (control) ->
    ...
    app.directive "rmaps#{control.dName}", (rmapsMapFactoryControlsLogger, $rootScope) -> control.directive(rmapsMapControlsLogger) unless $rootScope.silenceRmapsControls

# Leaflet usage:
#    rmapsControlsService.{Some}Control position: 'botomleft', scope: mapScope
app.service 'rmapsControlsService', ($compile, rmapsMapFactoryControlsLogger, $rootScope, $log) ->
  ...
  for control in directiveControls
    do (control) ->
      control.class = class extends L.Control
        ...
        initialize: (options) ->
          rmapsMapFactoryControlsLogger.debug "#{control.dName} init" unless $rootScope.silenceRmapsControls
          super options
        onAdd: (map) ->
          rmapsMapFactoryControlsLogger.debug "#{control.dName} onAdd" unless $rootScope.silenceRmapsControls
