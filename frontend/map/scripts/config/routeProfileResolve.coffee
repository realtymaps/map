...
currentIdentity
  ...
  # do we already have a `currentProfile`?
  profile = rmapsProfilesService.currentProfile
  if profile
    $log.debug "Set current profile to #{profile.id} via `rmapsProfilesService.currentProfile`"
    return profile

  # no `currentProfile`, grab from identity...
  else
